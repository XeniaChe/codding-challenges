...
const convToBinary = (int) => {
  ...
  while (curVal > 0) {
    ...
    curVal = Math.trunc(curVal / 2);
  }

  console.log({ arr });
  return arr; // !!!! Correct BINARY should be reversed
  // But not needed for this task
};
// convToBinary(21);

const task2 = (N) => {
  const notReversedBinary = convToBinary(N);

  let left = 0,
    maxLen = 0;

  while (left < notReversedBinary.length) {
    const cur = notReversedBinary[left];
    let right = left;

    if (cur == 1) {
      while (notReversedBinary[right + 1] == 0) {
        right++;
      }
...
const task3 = (A, K) => {
  ...
  for (let i = A.length - 1; i >= 0; i--) {
    const newInd = i + K >= A.length ? (i + K) % A.length : i + K;
...
const task4 = (A) => {
  ...
  for (const [key, val] of occurences) {
    if (val % 2) return key;
  }
};
...
let task5 = (X, Y, D, steps = 0) => {
  if (X >= Y) return steps;
  return task5(X + D, Y, D, ++steps);
};

task5 = (X, Y, D) => {
  const dif = Math.trunc((Y - X) / D);

  return X + dif * D >= Y ? dif : dif + 1;
};
